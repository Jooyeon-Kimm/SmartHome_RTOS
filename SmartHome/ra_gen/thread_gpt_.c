...
#if 1
static StaticTask_t thread_gpt__memory;
#if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t thread_gpt__stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t thread_gpt__stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread_gpt_") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
TaskHandle_t thread_gpt_;
void thread_gpt__create(void);
static void thread_gpt__func(void *pvParameters);
...
/* Actual period: 0.00065536 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 0, .channel = 6, .p_callback = NULL,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.00065536 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 0, .channel = 4, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
const rm_freertos_port_parameters_t thread_gpt__parameters =
{ .p_context = (void*) NULL, };
...
void thread_gpt__create(void)
{
...
#if 1
    thread_gpt_ = xTaskCreateStatic (
#else
                    BaseType_t thread_gpt__create_err = xTaskCreate(
                    #endif
                                     thread_gpt__func,
                                     (const char*) "Thread_gpt", 1024 / 4, // In words, not bytes
                                     (void*) &thread_gpt__parameters, //pvParameters
                                     1,
#if 1
                                     (StackType_t*) &thread_gpt__stack,
                                     (StaticTask_t*) &thread_gpt__memory
#else
                        & thread_gpt_
                        #endif
                                     );
    ...
    if (NULL == thread_gpt_)
    {
        rtos_startup_err_callback (thread_gpt_, 0);
    }
#else
                    if (pdPASS != thread_gpt__create_err)
                    {
                        rtos_startup_err_callback(thread_gpt_, 0);
                    }
                    #endif
...
static void thread_gpt__func(void *pvParameters)
{
    ...
    thread_gpt__entry (pvParameters);
}
